...
public class AdhChatControl extends AbstractController implements KeyInputHandler {
	...
	private static final String ADH_CHAT_BUTTON = "#adh-chat-text-button";
	...
	public boolean keyEvent(final NiftyInputEvent inputEvent) {
		if (Arrays.asList(NiftyStandardInputEvent.SubmitText, NiftyStandardInputEvent.Activate).contains(inputEvent)) {
			sendText();
			return true;
		}
		return false;
	...
	public void bind(@Nonnull final Nifty niftyParam, @Nonnull final Screen screenParam,
			@Nonnull final Element newElement, @Nonnull final Parameters properties) {
	...
	public final void sendText() {
		...
		Element button = getElement().findElementById(ADH_CHAT_BUTTON);
		if (button != null) {
			button.setFocus();
		}
		if (text.length() == 0) {
