...
public final class FontFactory {
    ...
    /**
     * Toggle whether fonts that are loaded are fingerprinted with a SHA-1 hash. If
     * a font is fingerprinted then a SHA-1 hash is generated at load time and
     * stored in the font. This is useful for uniquely identifying fonts. By default
     * this is turned on.
     *
     ...
     * than one font and in this case multiple font objects will be returned. If the
     * data in the stream cannot be parsed or is invalid an array of size zero will
     * be returned.
     *
     ...
     * settings on the factory are used during the loading process. One or more font
     * builders are returned if the stream contains valid font data. Some font
     * container formats may have more than one font and in this case multiple font
     * builder objects will be returned. If the data in the stream cannot be parsed
    ...
    /**
     * Load the font(s) from the byte array. The current settings on the factory are
     * used during the loading process. One or more fonts are returned if the stream
     * contains valid font data. Some font container formats may have more than one
     * font and in this case multiple font objects will be returned. If the data in
     * the stream cannot be parsed or is invalid an array of size zero will be
     * returned.
     *
    ...
    /**
     * Load the font(s) from the byte array into font builders. The current settings
     * on the factory are used during the loading process. One or more font builders
     * are returned if the stream contains valid font data. Some font container
     * formats may have more than one font and in this case multiple font builder
     * objects will be returned. If the data in the stream cannot be parsed or is
     * invalid an array of size zero will be returned.
     *
    ...
    private Builder loadSingleOTFForBuilding(WritableFontData wfd, int offsetToOffsetTable) throws IOException {
        if (this.fingerprintFont()) {
    ...
    private Builder[] loadCollectionForBuilding(WritableFontData wfd) throws IOException {
        wfd.readULongAsInt(Offset.TTCTag.offset);
        wfd.readFixed(Offset.Version.offset);
        ...
        for (int fontNumber = 0; fontNumber < numFonts; fontNumber++, offsetTableOffset += FontData.DataSize.ULONG
                .size()) {
     ...
     * given. Any tables whose id is not listed in the ordering will be placed in an
     * unspecified order following those listed.
     *
