...
abstract class ByteArray<T extends ByteArray<T>> {
     ...
     * Constructor.
     *
     * @param filledLength  the length that is "filled" and readable counting from
     *                      the offset
     * @param storageLength the maximum storage size of the underlying data
     * @param growable      is the storage growable - storageLength is the maximum
     *                      growable size
     */
     ...
     * bytes as will fit into the buffer are read unless that would go past the end
     * of the array.
     *
     * @param index the index into the byte array
    ...
    /**
     * Gets the bytes from the given index and fill the buffer with them starting at
     * the offset given. As many bytes as the specified length are read unless that
     ...
     * @throws IndexOutOfBoundsException if attempt to write outside the bounds of
     *                                   the data
     */
    ...
    /**
     * Puts the specified bytes into the array at the given index. The entire buffer
     * is put into the array unless that would extend beyond the length and the
     * array isn't growable.
     *
     * @param index the index into the byte array
     * @param b     the bytes to put into the array
     * @return the number of bytes actually written
     * @throws IndexOutOfBoundsException if the index for writing is outside the
     *                                   bounds of the data
     */
     ...
     * and the array isn't growable. The bytes to be put into the array are those in
     * the buffer from the given offset and for the given length.
     *
